...
<div class="add_edit" id="formadd" style="display: none" title="">
	<form autocomplete="on" method="post" id="form_add" name="form_add"
		...
		<fieldset>
			<ol>
				...
				<li><label for="Montant">Montant de la facture</label><input
					...
					id="Achat.Montant" name="Achat.Montant" class="calcMontant"
					/> </li>
			</ol>
		</fieldset>
		<fieldset>
			<ol>
				<li><label for="DateEcheance">Date d'échéance</label><input required
					type="date" id="Achat.DateEcheance" name="Achat.DateEcheance" /></li>
				<li><label for="Ristourne">Ristourne accordée</label><input required
					 step="0.01" type="number" id="Achat.Ristourne"
				...
				<li><label for="MontantPaye">Montant payé</label><input
					placeholder="$" required  step="0.01" type="number"
					id="Achat.MontantPaye" name="Achat.MontantPaye" class="mtpPlaceholder"/>
		...
		<div class="remarque_add">
			...
			<textarea id="Achat.Remarque" name="Achat.Remarque" class="textAreaRemarque" ></textarea>
		</div>
	...
	<form autocomplete="on" method="post" id="form_edit" name="form_edit"
		...
		<fieldset>
			<ol>
				...
				<li><label for="Montant">Montant de la facture</label><input
					placeholder="$" required  step="0.01" type="number"
					id="Achat.Montant_edit" name="Achat.Montant" class="cal_Montant_edit"/></li>
			</ol>
		</fieldset>
		<fieldset>
			<ol>
				<li><label for="DateEcheance">Date d'échéance</label><input required
					type="date" id="Achat.DateEcheance_edit" name="Achat.DateEcheance" /></li>
				<li><label for="Ristourne">Ristourne accordée</label><input required
					 step="0.01" type="number" id="Achat.Ristourne_edit"
		...
		<div class="remarque_edit">
			<label for="Remarque">Remarque</label>
			<textarea id="Achat.Remarque_edit" name="Achat.Remarque" class="textAreaRemarque"></textarea>
		</div>
		<!-- Allow form submission with keyboard without duplicating the dialog button -->
...
$mois=array('Janvier','Février','Mars','April','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre');
for($i = 0; $i < 12; ++ $i) {
