...
var months = { 'Janvier':1,'Février':2,'Mars':3,'April':4,'Mai':5,'Juin':6,'Juillet':7,'Août':8,'Septembre':9,'Octobre':10,'Novembre':11,'Décembre':12 };
$(document)
		.ready(
				function() {
					...
					$("#MySerach")
							.submit(
									function(event) {
										...
										var _month =months[$('#mySearch_month').val()];
										if(!_month ){
											_month="";
										}
										/*
					...
					function calcMontantEdit(){
						...
						if (Montant && Ristourne) {
							var MontantPaye = Montant
									- (Montant * Ristourne / 100);
							$("#formedit [name='Achat.MontantPaye']").val(MontantPaye);
													...
													.ajax({
														...
														success : function(data) {
															...
															$(
																	...
																	.val(
																			data['lastId']);
															if (!confirm('Le record a été ajouter \n Voulez-vous continuer?')) {
																dialogAdd
																		.dialog("close");
															}
					...
					function updateRecord() {
						...
						var Ristourne = $("#formedit [name='Achat.Ristourne']")
								.attr('value');
						var Remarque = escape($("#formedit [name='Achat.Remarque']")
								.attr('value'));
						if (!Remarque) {
							Remarque = ' ';
						}
						...
						$
								.ajax({
									...
									success : function(data) {
										$('#flex1').flexReload();
										if (!confirm('Le record a été modifier \n Voulez-vous continuer?')) {
